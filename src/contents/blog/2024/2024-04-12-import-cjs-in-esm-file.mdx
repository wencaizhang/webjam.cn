...
2. **文件路径：** `import()` 的参数是 CommonJS模块的文件路径, 应相对于当前模块或绝对路径, 如果这个 CJS 模块是 npm 包的话, 只提供包名即可.
...
如果你的环境支持顶层 `await`（Node.js v14.8.0起支持，在 ES2022 中被正式标准化），你可以在模块的顶层使用 `async/await` 来导入CommonJS模块，并且之后可以多次使用它。
...
async function useCjsFunction() {
  ...
  const cjsModule: { default: CjsModuleFunctionType } = await import('./cjs-module.js').catch(console.error);
...
这告诉TypeScript我们期望这个动态导入的模块有一个默认导出，其类型为 `CjsModuleFunctionType`, 但也有一些模块使用的是命名导出而不是默认导出，此时你可以这样指定类型：
...
async function useCjsFunction() {
  // 如果模块使用命名导出
  const { dynamicFunction }: { dynamicFunction: CjsModuleFunctionType } = await import('./cjs-module.js').catch(console.error);
